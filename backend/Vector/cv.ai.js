...
const EMBED_MODEL = "nvidia/nv-embedqa-e5-v5"; // embedding — 1024 dim، بيدعم input_type (query/passage)
const CHAT_MODEL = "openai/gpt-oss-20b";        // chat / تحليل الـ CV — كله على NVIDIA

// ══════════════════════════════════════════════════════════════════════════════
// NVIDIA HELPERS
// ══════════════════════════════════════════════════════════════════════════════
// ✅ اتعملها export عشان تتستخدم من company.ai.js برضو (بحث الشركات)
export async function getEmbeddings(texts, inputType = "passage") {
  try {
    console.log(
      "[DEBUG] NVIDIA_API_KEY loaded:",
      Env.NVIDIA_API_KEY ? `${Env.NVIDIA_API_KEY.slice(0, 12)}... (length: ${Env.NVIDIA_API_KEY.length})` : "❌ MISSING/UNDEFINED"
    );
    console.log("[DEBUG] EMBED_MODEL:", EMBED_MODEL);
    console.log("[DEBUG] texts being sent:", JSON.stringify(texts).slice(0, 200));
    ...
    const response = await fetch(`${NVIDIA_BASE_URL}/embeddings`, {
      ...
      body: JSON.stringify({
        model: EMBED_MODEL,
        input: texts,
        input_type: inputType, // ✅ nv-embedqa-e5-v5 محتاج الباراميتر ده فعلياً: "query" أو "passage"
...
export async function embedAndStore(cvId, text) {
  ...
  // ⚠️ Upstash Vector بيستخدم eventual consistency — الـ upsert بيرجع نجاح
  // فوراً بس الـ vectors ممكن ماتبقاش متاحة للـ query على طول. بننتظر شوية
  // قبل ما نسمح لأي queryCV يجري على الـ vectors دي عشان نتجنب نتايج فاضية.
  await new Promise((r) => setTimeout(r, 1500));

  return { vectorIds: records.map((r) => r.id), embeddingTokens: totalEmbeddingTokens };
}

export async function queryCV(cvId, { maxRetries = 4, retryDelayMs = 1000 } = {}) {
  const query = "skills experience education projects certifications";
  ...
  // ⚠️ Upstash Vector بيستخدم eventual consistency — أحياناً الـ vectors
  // اللي اتعملها upsert لسه ماوصلتش للـ index وقت أول query. بنعيد المحاولة
  // كذا مرة قبل ما نستسلم ونرجع context فاضي.
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    // Upstash بيستخدم filter كـ SQL-like string مش object زي Pinecone
    result = await vectorIndex.query({
      vector,
      topK: 10,
      includeMetadata: true,
    ...
    console.warn(
      `[queryCV] attempt ${attempt}/${maxRetries} رجّع صفر matches لـ cvId: ${cvId}، بننتظر ونعيد المحاولة...`
    );
...
//         input: texts,
//         encoding_format: "float",
//         truncate: "NONE", // ⚠️ NVIDIA بيرفض "END" لموديل bge-m3 — لازم "NONE"
//         // ⚠️ input_type اتشال: مش باراميتر مدعوم لموديل baai/bge-m3 على NVIDIA،
//         // وإرساله كان بيخلي الـ API يرجّع 500 على كل استدعاء
//       }),
//     });
...
//   // ⚠️ NVIDIA بيرفض input_type: "query" مع الموديل/الحساب ده (بيرجع 500)
//   const embData = await getEmbeddings([query], "passage");
//   const vector = embData.embeddings[0];
