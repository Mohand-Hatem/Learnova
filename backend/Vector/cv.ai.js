...
export async function searchCandidates(query, topK = 10) {
  // 1. Convert the company's search query to a vector
  const queryVector = await embeddings.embedQuery(query);

  // 2. Search Pinecone for matching CV chunks
  const result = await pineconeIndex.query({
    vector: queryVector,
    topK: topK * 3, // نجيب أكتر عشان نعمل deduplication
    includeMetadata: true,
  });

  if (!result.matches?.length) return [];

  // 3. Deduplicate — نرجع أعلى score لكل CV
  const cvScoreMap = new Map();
  for (const match of result.matches) {
    const cvId = match.metadata?.cvId;
    if (!cvId) continue;
    const existing = cvScoreMap.get(cvId);
    if (!existing || match.score > existing.score) {
      cvScoreMap.set(cvId, { cvId, score: match.score });
    }
  }

  // 4. Sort by score and take topK
  const topCvs = [...cvScoreMap.values()]
    .sort((a, b) => b.score - a.score)
    .slice(0, topK);

  return topCvs; // [{ cvId, score }]
}
