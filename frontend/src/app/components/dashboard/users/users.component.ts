...
export class UsersComponent implements OnInit {
  ...
  // keep paginatedUsers pointing to users() since pagination is now server-side
  ...
  loadUsers() {
    ...
    if (this.bannedFilter()) params['isBanned'] = this.bannedFilter() === 'banned' ? 'true' : 'false';
...
private computeMenuAnchor(trigger: HTMLElement): { top: number; left: number } {
  ...
  requestPlanChange(u: UserItem, plan: string, event?: Event) {
    ...
    dialogRef.afterClosed().subscribe((confirmed) => {
      if (confirmed) this.updatePlanInBackend(u._id, plan);
    });
  }
