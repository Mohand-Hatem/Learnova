...
} from '../admins/create-admin-dialog/create-admin-dialog.component';
...
export class CompaniesComponent implements OnInit {
  ...
  loadCompanies() {
    ...
    this.companyService.getAllCompanies(params).subscribe({
      next: (res) => {
        ...
        this.totalPagesCount.set(res.totalPages);
        this.loading.set(false);
      },
      error: () => this.loading.set(false),
    });
  ...
  getTokenPct(c: CompanyItem): number {
    const tokenLimit = Number(c.maxToken) || 0;
    if (!tokenLimit) return 0;
    return Math.min(100, Math.round((Math.max(0, Number(c.tokenUsage) || 0) / tokenLimit) * 100));
  }
    ...
    dialogRef.afterClosed().subscribe((confirmed) => {
      ...
      this.companyService.deleteCompany(id).subscribe({
        next: () => {
          this.loadCompanies();
          this.closeMenu();
          this.toastr.success('Company deleted successfully.', 'Deleted');
          if (c) this.sessionNotifications.add(`Admin deleted company ${this.getName(c)}`, 'error');
        },
        error: (err) => {
          this.toastr.error(err.error?.message || 'Could not delete company.', 'Delete failed');
        },
      });
    });
  }
